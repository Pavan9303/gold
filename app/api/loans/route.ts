...
export async function GET(req: NextRequest) {
  ...
  for (const lid of loanIds) {
    ...
    if (data) {
      ...
      loans.push({ ...loan, calculatedInterest: calculateLoanInterest(loan) });
    }
  }
...
export async function POST(req: NextRequest) {
  ...
  const { customerId, items, principalAmount, interestRate, startDate, dueDate, durationPreset, isCompoundInterest, notes } = body;
  ...
  const loan: Loan = {
    ...
    isCompoundInterest: isCompoundInterest === true,
